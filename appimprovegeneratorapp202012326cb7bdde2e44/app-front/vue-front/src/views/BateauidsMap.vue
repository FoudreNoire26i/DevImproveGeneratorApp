  ...
  <v-container fluid pa-0>
    <v-bottom-sheet v-model="sheet">
      <v-list two-line>
        <v-list-tile avatar>
          <v-list-tile-content>
            <v-list-tile-title>{{ point.nombateau_title }}</v-list-tile-title>
            <v-list-tile-sub-title>{{ point.modele_subtitle }}</v-list-tile-sub-title>
          </v-list-tile-content>
        </v-list-tile>
      </v-list>
    </v-bottom-sheet>
    <gmap-map ref="mymap" :center="center" :options="options" :zoom="6" style="width: 100vw;height: 89vh">
      <gmap-marker
        ...
        :key="key"
        :position="{lat: parseFloat(item.localisationport_geo_lat), lng: parseFloat(item.localisationport_geo_long) }"
        :clickable="true"
...
const apiUrl = `${constants.apiUrl}bateauids/`;
