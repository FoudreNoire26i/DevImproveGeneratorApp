  ...
  <v-container fluid pa-0>
    <v-bottom-sheet v-model="sheet">
      <v-list two-line>
        <v-list-tile avatar>
          <v-list-tile-avatar>
            <img :src="point.photo_img">
          </v-list-tile-avatar>
          <v-list-tile-content>
            <v-list-tile-title>{{ point.nomport_title }}</v-list-tile-title>
            <v-list-tile-sub-title>{{ point.numeroplace_subtitle }}</v-list-tile-sub-title>
          </v-list-tile-content>
        </v-list-tile>
      </v-list>
    </v-bottom-sheet>
    <gmap-map ref="mymap" :center="center" :options="options" :zoom="6" style="width: 100vw;height: 89vh">
      <gmap-marker
        ...
        :key="key"
        :position="{lat: parseFloat(item.localisation_geo_lat), lng: parseFloat(item.localisation_geo_long) }"
        :clickable="true"
